...
export default function ExtratoCFechamentoPage() {
  ...
  const todosSelec = registrosFiltrados.length > 0 && registrosFiltrados.every((r) => selecionados.has(r.extrato_id));
  const algumSelec = registrosFiltrados.some((r) => selecionados.has(r.extrato_id));

  const toggleAll = () => {
    if (todosSelec) {
      setSelecionados((prev) => {
        const next = new Set(prev);
        registrosFiltrados.forEach((r) => next.delete(r.extrato_id));
        return next;
      });
    } else {
      setSelecionados((prev) => {
        const next = new Set(prev);
        registrosFiltrados.forEach((r) => next.add(r.extrato_id));
        return next;
      });
    }
  };
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h1 className={`${headingFont.className} text-2xl font-extrabold tracking-tight text-slate-950 sm:text-3xl`}>
                {isAdmin ? "Extrato de Fechamento" : "Meus Direitos — Extrato de Comissões"}
              </h1>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
              ...
              <div className="overflow-x-auto">
                <div className="min-w-[720px]">
                  {/* Header */}
                  <div className={`grid ${isAdmin ? "grid-cols-[2.5rem_1.6fr_1fr_1fr_1fr_1fr_1fr]" : "grid-cols-[2.5rem_1.6fr_1fr_1fr_1fr_1fr]"} items-center bg-slate-50 px-4 py-2 text-xs font-bold uppercase tracking-wide text-slate-500`}>
                    ...
                    <div>Cliente</div>
                    {isAdmin && <div>Responsável</div>}
                    <div>Administradora</div>
                    <div title="Data em que o valor do acordo foi inserido no sistema">Data do acordo</div>
                    <div className="text-right">Acordo</div>
                    ...
                    {registrosFiltrados.map((r) => {
                      ...
                      return (
                        <div
                          ...
                          className={`grid cursor-pointer ${isAdmin ? "grid-cols-[2.5rem_1.6fr_1fr_1fr_1fr_1fr_1fr]" : "grid-cols-[2.5rem_1.6fr_1fr_1fr_1fr_1fr]"} items-center gap-2 px-4 py-3 text-sm transition select-none ${sel ? "bg-emerald-50" : "hover:bg-slate-50"}`}
                        >
                          <div className="flex items-center justify-center">
                            {sel
                              ? <CheckSquare className="h-4 w-4 text-emerald-600" />
                          ...
                          <div title="Data em que o valor do acordo foi inserido">
                            <div className="font-semibold text-slate-700">{fmtDataBR(r.data_valor_acordo)}</div>
                            <div className="text-[10px] font-semibold text-slate-400">{fmtHoraBR(r.data_valor_acordo)}</div>
                            {r.data_estimada && (
                              <div className="text-[10px] font-semibold text-amber-600">estimada</div>
                            )}
                          </div>
                          ...
                          <div className="text-right">
                            <div className="font-extrabold text-emerald-800">{fmtBRL(r.valor_comissao)}</div>
                  ...
                  {registrosFiltrados.length > 0 && (
                    <div className={`grid ${isAdmin ? "grid-cols-[2.5rem_1.6fr_1fr_1fr_1fr_1fr_1fr]" : "grid-cols-[2.5rem_1.6fr_1fr_1fr_1fr_1fr]"} items-center border-t-2 border-slate-200 bg-slate-50 px-4 py-3 text-sm font-extrabold text-slate-900`}>
                      <div />
                      <div>{registrosFiltrados.length} acordo(s)</div>
                      {isAdmin && <div />}
                      <div />
                      <div />
                      <div className="text-right">
                        {fmtBRL(registrosFiltrados.reduce((s, r) => s + r.valor_acordo, 0))}
                      </div>
