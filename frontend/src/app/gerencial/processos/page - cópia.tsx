...
function getDocsStatus(row: any): { hasAddr: boolean; hasId: boolean; count: number; needs: boolean } {
  ...
  const docEntries = collectStrings(
    ...
    statusExtras?.documento_identidade,
    dbExtras?.documento_identidade,
  ...
  for (const candidate of minimosCandidates) {
    ...
    const ok = candidate.ok ?? candidate.minimos_ok ?? candidate.ready;
    if (ok === true) {
      hasAddr = true;
      hasId = true;
      break;
    }
    if (enderecoOK !== undefined) hasAddr = Boolean(enderecoOK);
    if (identidadeOK !== undefined) hasId = Boolean(identidadeOK);
  }
...
const fmtDateTime = (isoOrDate?: string|Date|null) => {
  ...
  return d.toLocaleString("pt-BR", { hour: '2-digit', minute: '2-digit', year: 'numeric', month: '2-digit', day: '2-digit' });
};
...
function getDisplayStatus(row: any): string {
  // Primeiro verifica se foi marcado como assinado externamente
...
function getResultadoLabel(it: any): string {
  ...
  if (r === "ganhamos") return "Sentença (Futuro)";
...
function getTimestampForStatus(row: any): string | null {
  const status = getDisplayStatus(row);
  // Lógica simplificada: O backend agora envia o campo 'timer_event_at'
  // que é a fonte única da verdade para o início do timer.
  if (status === 'Enviado' || status === 'Assinado' || status === 'Assinado (fora)') {
    return row?.timer_event_at || null;
  }
  return null; // Nenhum outro status deve iniciar um timer
}
...
function formatDurationDHMS(ms: number): string {
  // Debug para valores negativos
  if (ms < 0 && typeof window !== 'undefined' && window.location.href.includes('debug=1')) {
    console.log('⚠️ formatDurationDHMS recebeu valor negativo:', ms);
  }
  if (ms < 0) ms = 0;
...
function computeProcessTimeline(it: any, now: Date) {
  // NOVA LÓGICA: usar apenas os campos de timer do backend (versão robusta)
  const extras = it?.extras || {};
  const assinaturaStart = extras?.timer_assinatura_start || null;
  const assinaturaEnd = extras?.timer_assinatura_end || null;
  const anexosStart = extras?.timer_anexos_started_at || null;
  const anexosEnd = extras?.timer_anexos_ended_at || null;
  const advogadoStart = extras?.timer_advogado_start || null;
  const advogadoEnd = extras?.timer_advogado_end || null;
  ...
  // FASE 1: ASSINATURA
  if (assinaturaStart) {
    const startDate = new Date(assinaturaStart);
    const endDate = assinaturaEnd ? new Date(assinaturaEnd) : now;
    const duration = endDate.getTime() - startDate.getTime();
    const live = !assinaturaEnd;
    items.push({
      key: "assinatura",
      live,
      label: `${live ? "Aguardando Assinatura" : "✅ Assinatura"}: ${formatDurationDHMS(duration)}`,
      start: startDate,
      end: endDate,
      className: `ring-1 ring-inset rounded-lg px-2 py-1 text-xs font-semibold ${
        live ? "bg-sky-200 text-sky-900 ring-sky-700/30 animate-pulse" : "bg-green-100 text-green-700 ring-green-600/20"
      }`
    });
  ...
  // FASE 2: GERENTE / ANEXOS
  if (anexosStart) {
    const startDate = new Date(anexosStart);
    const endDate = anexosEnd ? new Date(anexosEnd) : now;
    const duration = endDate.getTime() - startDate.getTime();
    const live = !anexosEnd;
    items.push({
      key: "gerente", // Mantendo a chave para não quebrar estilos
      live,
      label: `${live ? "🔄 Gerente anexando" : "✅ Gerente"}: ${formatDurationDHMS(duration)}`,
      start: startDate,
      end: endDate,
      className: `ring-1 ring-inset rounded-lg px-2 py-1 text-xs font-semibold ${
        live ? "bg-amber-200 text-amber-900 ring-amber-700/30 animate-pulse" : "bg-green-100 text-green-700 ring-green-600/20"
      }`
    });
    if (!totalEnd || endDate > totalEnd) totalEnd = endDate;
  }

  // FASE 3: ADVOGADO
  if (advogadoStart) {
    const startDate = new Date(advogadoStart);
    const endDate = advogadoEnd ? new Date(advogadoEnd) : now;
    const duration = endDate.getTime() - startDate.getTime();
    const live = !advogadoEnd;
    items.push({
      key: "advogado",
      live,
      label: `${live ? "⚖️ Advogado trabalhando" : "🎉 Finalizado"}: ${formatDurationDHMS(duration)}`,
      start: startDate,
      end: endDate,
      className: `ring-1 ring-inset rounded-lg px-2 py-1 text-xs font-semibold ${
        live ? "bg-emerald-200 text-emerald-900 ring-emerald-700/30 animate-pulse" : "bg-green-100 text-green-700 ring-green-600/20"
      }`
    });
    if (!totalEnd || endDate > totalEnd) totalEnd = endDate;
  }

  const totalLabel = totalStart && totalEnd ?
    `Total: ${formatDurationDHMS(totalEnd.getTime() - totalStart.getTime())}` :
    undefined;

  return { items, totalLabel };
}

function getProcessTimestamps(it: any) {
  const extras = it?.extras || {};
  // FLUXO SEQUENCIAL OBRIGATÓRIO - FRONTEND IMPLEMENTADO:
  // 1. Sistema envia → timer_assinatura_start
  // 2. Cliente assina → timer_assinatura_end + timer_gerente_start
  // 3. Gerente "Enviar Advogado" → timer_gerente_end + timer_advogado_start  
  // 4. Advogado número processo → timer_advogado_end

  // USAR APENAS CAMPOS NORMALIZADOS (sem fallbacks)
  const startEnvio = extras?.timer_assinatura_start || null;
  const assinaturaConcluida = extras?.timer_assinatura_end || null;  
  const enviadoAdvogado = extras?.timer_gerente_end || null;
  const advogadoConcluiu = extras?.timer_advogado_end || null;

  // Validar consistência: apenas extratos ASSINADOS devem ter timer gerente
  const zapsignAssinado = it?.zapsign_status === 'assinado';
  const assinadoExterno = extras?.signed_external === true;
  const realmenteAssinado = zapsignAssinado || assinadoExterno;

  let statusLabel = '';
  try {
    statusLabel = getDisplayStatus(it);
  } catch {}

  return {
    startEnvio,
    assinaturaConcluida: realmenteAssinado ? assinaturaConcluida : null,
    enviadoAdvogado: realmenteAssinado ? enviadoAdvogado : null, 
    advogadoConcluiu: realmenteAssinado ? advogadoConcluiu : null,
    status: statusLabel,
    realmenteAssinado,
    // Dados para debug
    zapsignStatus: it?.zapsign_status,
    assinadoExterno: extras?.signed_external
  };
}
...
function MobileProcessCard({ it, compact=false, onToggleSignedExternal, onDelete }: { it: any; compact?: boolean; onToggleSignedExternal?: (p:any)=>void; onDelete?: (id:number)=>void; }) {
  ...
  const timeline = computeProcessTimeline(it, new Date());
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className={cls("font-semibold text-slate-900 truncate", compact && "text-sm")}>
            {it.nome_cliente || "—"}
          </div>
          {comarcaUF && (
            <div className={cls("mt-0.5 text-[11px] text-slate-500 truncate", compact && "text-[10px]")} title={comarcaUF}>
          ...
          <div className={cls("mt-1 text-xs text-slate-600 flex flex-col gap-1", compact && "text-[11px]")}>
            <div className="flex items-center gap-2 flex-wrap">
              <span>Proc.: <span className="font-semibold text-slate-900">{numeroProc || "—"}</span></span>
              <span>Grupo/Cota: <span className="font-medium">{it.grupo ?? "—"}/{it.cota ?? "—"}</span></span>
            </div>
            {comarcaNome && (
              <div className={cls("text-[11px] text-slate-500", compact && "text-[10px]")} title={comarcaNome}>
                {comarcaNome}
              </div>
            )}
            <div className="mt-2 bg-linear-to-r from-blue-50 to-indigo-50 rounded-xl p-3 border border-blue-200 shadow-md">
              <style dangerouslySetInnerHTML={{
                __html: `
                  .phase-container div:contains('Fase: Gerente anexando'),
                  .phase-container span:contains('Fase: Gerente anexando'),
                  .phase-container p:contains('Fase: Gerente anexando'),
                  .phase-container *:contains('Fase: Gerente anexando') {
              ...
              <div className="phase-container">
                <ProcessTimeline 
                  {...getProcessTimestamps(it)} 
                  timers={{
                    assinatura: {
                      started_at: it?.extras?.timer_assinatura_start || null,
                      ended_at: it?.extras?.timer_assinatura_end || null,
                      active: !!(it?.extras?.timer_assinatura_start && !it?.extras?.timer_assinatura_end)
                    },
                    anexos: {
                      started_at: it?.extras?.timer_gerente_start || null,
                      ended_at: it?.extras?.timer_gerente_end || null,
                      active: !!(it?.extras?.timer_gerente_start && !it?.extras?.timer_gerente_end)
                    },
                    advogado: {
                      started_at: it?.extras?.timer_advogado_start || null,
                      ended_at: it?.extras?.timer_advogado_end || null,
                      active: !!(it?.extras?.timer_advogado_start && !it?.extras?.timer_advogado_end)
                    }
                  }} 
                />
              </div>
        ...
        <div className="text-right">
          ...
          {docs.count > 0 && (
            <div className={cls("mt-0.5 text-[10px]", 
              awaiting && statusLabel !== "Assinado" ? "text-amber-700" : "text-slate-500"
            )}>
              {docs.count} arquivo(s)
            </div>
          )}
          <div className="mt-2 flex gap-2 justify-end">
            <button 
              onClick={() => onToggleSignedExternal && onToggleSignedExternal(it)} 
              className={`text-[12px] ${statusLabel === 'Assinado (fora)' ? 'text-slate-500' : 'text-emerald-700'} underline`}
            >
              {statusLabel === 'Assinado (fora)' ? 'Remover Assinado Fora' : 'Assinado Fora'}
            </button>
          </div>
        </div>
      </div>
      ...
      {!compact && (
        <>
          <div className="mt-3 grid grid-cols-2 gap-2">
            ...
            <div className="rounded-xl bg-slate-50 p-2">
              <div className="text-[10px] text-slate-500">Honorários Futuro (tot.)</div>
              {(() => {
...
export default function GerencialProcessosPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* Conteúdo */}
      <main className="mx-auto max-w-none px-4 pb-24 pt-3 sm:px-6 lg:px-8">
        {/* 🔵 Filtros — Mobile simplificado / Desktop completo */}
        <div className="mb-4 space-y-4">
          {/* MOBILE */}
          <div className="md:hidden space-y-2">
            <div className="relative">
              <input
                className="w-full rounded-xl border border-slate-300 bg-white pl-9 pr-3 py-2 text-sm outline-none ring-0 focus:border-slate-400"
                placeholder="Buscar ID, cliente, processo…"
                value={q}
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={cls(
                  "flex-1 rounded-lg px-3 py-2 text-xs font-semibold border",
                  mobileMode==="cards" ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-700 border-slate-300"
                )}
              >
                Cards
              </button>
              <button
                ...
                className={cls(
                  "flex-1 rounded-lg px-3 py-2 text-xs font-semibold border",
                  mobileMode==="compact" ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-700 border-slate-300"
                )}
              >
                Compacto
              </button>
            ...
            <div className="flex items-center gap-2">
              <label className="text-xs text-slate-600">Mostrar só em andamento</label>
              <input type="checkbox" className="h-4 w-4" checked={onlyLive} onChange={(e)=>{setOnlyLive(e.target.checked); setPage(1);}} />
            </div>

            {/* Filtros essenciais */}
            <div className="grid grid-cols-2 gap-2 text-xs">
              <select className="rounded-xl border border-slate-300 bg-white px-2 py-2" value={status} onChange={(e)=>{setStatus(e.target.value); setPage(1);}}>
                <option value="">Status</option>
                {uniqueStatuses.map((s)=> <option key={s} value={s}>{s}</option>)}
              </select>
              <select className="rounded-xl border border-slate-300 bg-white px-2 py-2" value={resultado} onChange={(e)=>{setResultado(e.target.value); setPage(1);}}>
                <option value="">Resultado</option>
                {uniqueResultados.map((s)=> <option key={s} value={s}>{s}</option>)}
              </select>
              <select className="rounded-xl border border-slate-300 bg-white px-2 py-2" value={adm} onChange={(e)=>{setAdm(e.target.value); setPage(1);}}>
                <option value="">Administradora</option>
                {uniqueAdms.map((s)=> <option key={s} value={s}>{s}</option>)}
              </select>
              <select className="rounded-xl border border-slate-300 bg-white px-2 py-2" value={advogadoFilter} onChange={(e)=>{setAdvogadoFilter(e.target.value); setPage(1);}}>
                <option value="">Advogado</option>
                {uniqueAdvogados.map((s)=> <option key={s} value={s}>{s}</option>)}
              </select>
              <input type="date" className="rounded-xl border border-slate-300 bg-white px-2 py-2" value={dateFrom} onChange={(e)=>{setDateFrom(e.target.value); setPage(1);}} />
              <input type="date" className="rounded-xl border border-slate-300 bg-white px-2 py-2" value={dateTo} onChange={(e)=>{setDateTo(e.target.value); setPage(1);}} />
            </div>
            <div className="flex items-center justify-between text-xs">
              <button onClick={clearFilters} className="underline text-slate-600">Limpar filtros</button>
              <div className="flex items-center gap-2">
                <button onClick={exportCSV} className="inline-flex items-center gap-1 rounded-xl bg-slate-900 px-3 py-2 text-xs font-medium text-white shadow-sm hover:bg-slate-800">
          ...
          <div className="hidden md:block">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <label className="flex items-center gap-2 text-sm text-slate-600">
                  <input type="checkbox" className="h-4 w-4" checked={isCompactTable} onChange={(e)=> setIsCompactTable(e.target.checked)} />
                  Compacta
                </label>
                <button
        ...
        <div className="md:hidden mb-3 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-3 sm:px-4 py-3 text-xs sm:text-sm shadow-sm">
          <div className="text-slate-700">
            Resultados filtrados: <span className="font-semibold">{total}</span> processo(s)
          </div>
          <div className="flex items-end gap-3 sm:gap-4 text-slate-700">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <span className="rounded-full border px-2.5 py-1.5 text-[11px] sm:text-xs bg-white">
                {totals.hasHoje ? `Hoje: ${fmtBRL(totals.sumHoje)}` : "Hoje: —"}
              </span>
              <span className="rounded-full border px-2.5 py-1.5 text-[11px] sm:text-xs bg-white">
                {totals.hasFuturo ? `Futuro: ${fmtBRL(totals.sumFuturo)}` : "Futuro: —"}
            ...
            <div className="hidden sm:flex flex-col items-stretch gap-2">
              <Link
                href="/"
                className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-3 py-2 text-xs sm:text-sm font-medium text-white shadow-sm hover:bg-emerald-500"
                title="Cadastrar novo processo"
              >
              ...
              <button
                onClick={exportCSV}
                className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-3 py-2 text-xs sm:text-sm font-medium text-white shadow-sm hover:bg-slate-800"
        ...
        {/* ====== MOBILE: Lista em cartões (duas densidades) ====== */}
        <section className="md:hidden space-y-2">
          ...
          {!loading && pageItems.map((it)=> (
            <MobileProcessCard key={it.id} it={it} compact={mobileMode==="compact"} onToggleSignedExternal={toggleSignedExternal} onDelete={deleteProcess} />
          ))}
        </section>
                ...
                <thead className="text-xs">
                  <tr className="bg-white sticky top-0 z-10 border-b text-slate-600">
                    {[
                      ...
                      { key: "_aguardando_adv", label: "Etapas:", w: "w-[180px]" },
      ...
      <Link
        href="/"
        className="md:hidden fixed right-4 bottom-24 z-30 rounded-full p-3 shadow-lg bg-emerald-600 text-white"
        title="Cadastrar novo processo"
      >
