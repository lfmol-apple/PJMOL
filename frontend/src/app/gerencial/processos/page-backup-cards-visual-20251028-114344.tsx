...
function getStatusJulgamento(it: any): string {
  const numeroProc = (it.numero_processo ?? it.numeroProcesso ?? it.processo_numero ?? "").toString().trim();
  if (!numeroProc) return "";
  
  const resultado = String(it?.resultado_processo || "").toLowerCase();
  if (resultado === "acordo") return "Acordo";
  if (resultado === "ganhamos" && isAvista(it?.tipo_pagamento)) return "Sentença à Vista";
  if (resultado === "ganhamos") return "Sentença Futura";
...
function MobileProcessCard({ it, compact=false, onToggleSignedExternal, onDelete }: { it: any; compact?: boolean; onToggleSignedExternal?: (p:any)=>void; onDelete?: (id:number)=>void; }) {
  ...
  const numeroProc = (it.numero_processo ?? it.numeroProcesso ?? it.processo_numero ?? "").toString().trim();
  const comarcaUF = displayComarcaUF(it);
  const comarcaNome = displayComarcaNome(it);
  const statusLabel = getDisplayStatus(it);
  const statusJulgamento = getStatusJulgamento(it);
  ...
  return (
    ...
    >
      {/* TOPO: Cliente, Nº Processo, Status */}
      <div className={cls("p-3 border-b", awaiting ? "bg-amber-50/50" : "bg-slate-50/50")}>
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="min-w-0 flex-1">
            ...
            <div className="text-xs text-slate-600 mt-0.5">
              Proc.: <span className="font-semibold text-slate-900">{numeroProc || "—"}</span>
            </div>
          </div>
          ...
          >
            ...
            {docs.count > 0 && <span className="text-[10px]">({docs.count})</span>}
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap mt-2">
          <span className={cls(
            ...
            statusLabel === "Assinado" ? "bg-emerald-100 text-emerald-800" :
            statusLabel === "Assinado (fora)" ? "bg-blue-100 text-blue-800" :
            statusLabel === "Aguardando Assinatura" ? "bg-amber-100 text-amber-800" :
      ...
      {/* CORPO: Todas as outras informações */}
      <div className="p-3 space-y-3">
        {/* Localização */}
        <div className="space-y-1">
          {comarcaUF && (
            <div className="text-xs text-slate-600">
              <span className="font-medium text-slate-500">Comarca:</span> {comarcaUF}
            </div>
          )}
          {comarcaNome && (
            <div className="text-xs text-slate-500 truncate" title={comarcaNome}>
              {comarcaNome}
            </div>
          )}
        </div>

        {/* Grupo/Cota e Administradora */}
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div>
            <span className="text-slate-500">Grupo/Cota:</span>
            <div className="font-semibold text-slate-900">{it.grupo ?? "—"}/{it.cota ?? "—"}</div>
          </div>
          <div className="truncate">
            <span className="text-slate-500">Adm:</span>
            <div className="font-medium text-slate-900 truncate" title={it.administradora || ''}>
              {it.administradora || "—"}
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div className="truncate">
            <span className="text-slate-500">Advogado:</span>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-lg bg-blue-50 p-2 border border-blue-200">
            <div className="text-[10px] font-medium text-blue-700 mb-0.5">Valor Hoje</div>
            <div className="text-sm font-bold text-blue-900 tabular-nums">
              {fmtBRL(valorHojeDisplay(it))}
            </div>
          </div>
          <div className="rounded-lg bg-purple-50 p-2 border border-purple-200">
            <div className="text-[10px] font-medium text-purple-700 mb-0.5">Valor Futuro</div>
            <div className="text-sm font-bold text-purple-900 tabular-nums">
              {fmtBRL((it.valor_futuro ?? it.valor_corrigido_futuro))}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <div className="rounded-lg bg-emerald-50 p-2 border border-emerald-200">
            <div className="text-[10px] font-medium text-emerald-700 mb-1">Honorários Hoje</div>
            {(() => {
              const adv = Number(it.honorarios_hoje_adv||0);
              const emp = Number(it.honorarios_hoje_emp||0);
              const tot = Number((it.honorarios_hoje_total ?? (adv+emp)) ?? 0) || 0;
              return (
                <>
                  <div className="text-sm font-bold text-emerald-900 tabular-nums">{fmtBRL(tot)}</div>
                  <div className="text-[10px] text-emerald-600 mt-0.5">
                    Adv: {fmtBRL(adv)} + Emp: {fmtBRL(emp)}
                  </div>
                </>
              );
            })()}
          ...
          <div className="rounded-lg bg-teal-50 p-2 border border-teal-200">
            <div className="text-[10px] font-medium text-teal-700 mb-1">Honorários Futuro</div>
            {(() => {
              const adv = Number(it.honorarios_futuro_adv||0);
              const emp = Number(it.honorarios_futuro_emp||0);
              const tot = Number((it.honorarios_futuro_total ?? (adv+emp)) ?? 0) || 0;
              return (
                <>
                  <div className="text-sm font-bold text-teal-900 tabular-nums">{fmtBRL(tot)}</div>
        ...
        <Link
          ...
          className="block w-full text-center rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-slate-800 transition-colors"
        >
          VER EXTRATO
        </Link>
      </div>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <span className="text-sm font-semibold text-slate-700">
                {currentUser?.nome || currentUser?.email || 'Usuário'}
              </span>
              {perfil && (
          ...
          {!loading && mobileMode === "compact" && pageItems.map((it)=> {
            ...
            return (
              <div key={it.id} className={cls(
                ...
                awaiting ? "border-amber-300 bg-amber-50/30" : "border-slate-200"
              )}>
                {/* Linha: Cliente e Anexos */}
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Link
                    ...
                    className={cls(
                      ...
                      awaiting ? "border-amber-400 bg-amber-100 text-amber-800" : "border-slate-300 bg-white text-slate-700"
                    )}
                  >
                    <Paperclip className="h-3 w-3" />
                    {awaiting ? "Pendente" : "Anexos"}
                    {docs.count > 0 && <span className="text-[10px]">({docs.count})</span>}
                  </Link>
                </div>

                {/* Linha: Processo e Status */}
                ...
                {/* Linha: Resultado e Comarca */}
                <div className="flex items-center justify-between gap-2 text-xs">
                  <div>
                    <span className="text-slate-500">Resultado:</span>{" "}
                    <span className="font-medium text-slate-900">{resLabel || "—"}</span>
                  </div>
                  <div className="text-slate-600 truncate max-w-[140px]" title={comarcaUF}>
                    {comarcaUF || "—"}
                  </div>
                </div>

                {/* Linha: Grupo/Cota e Administradora */}
                <div className="flex items-center justify-between gap-2 text-xs">
                  <div>
                    <span className="text-slate-500">Grupo/Cota:</span>{" "}
                    <span className="font-medium text-slate-900">{it.grupo ?? "—"}/{it.cota ?? "—"}</span>
                  </div>
                  <div className="text-slate-600 truncate max-w-[140px]" title={it.administradora || ''}>
                    {it.administradora || "—"}
                  </div>
                </div>

                {/* Linha: Advogado e Gerente */}
                <div className="flex items-center justify-between gap-2 text-xs">
                  <div className="truncate">
                    <span className="text-slate-500">Adv:</span>{" "}
                    <span className="font-medium text-slate-900">{displayAdvogadoName(it) || "—"}</span>
                  </div>
                  <div className="text-slate-600 truncate max-w-[120px]" title={displayGerenteName(it) || ''}>
                    {displayGerenteName(it) || (it.gerente_id ? `#${it.gerente_id}` : "—")}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-200">
                  <div>
                    <div className="text-slate-500 text-[10px]">Valor Hoje</div>
                    <div className="font-bold text-slate-900 tabular-nums">{fmtBRL(valorHojeDisplay(it))}</div>
                  </div>
                  <div>
                    <div className="text-slate-500 text-[10px]">Valor Futuro</div>
                    <div className="font-bold text-slate-900 tabular-nums">{fmtBRL((it.valor_futuro ?? it.valor_corrigido_futuro))}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <div className="text-slate-500 text-[10px]">Hon. Hoje</div>
                    <div className="font-semibold text-slate-900 tabular-nums">
                      {fmtBRL(Number((it.honorarios_hoje_total ?? (Number(it.honorarios_hoje_adv||0) + Number(it.honorarios_hoje_emp||0))) ?? 0) || 0)}
                    </div>
                  </div>
                  <div>
                    <div className="text-slate-500 text-[10px]">Hon. Futuro</div>
                    <div className="font-semibold text-slate-900 tabular-nums">
                      {fmtBRL(Number((it.honorarios_futuro_total ?? (Number(it.honorarios_futuro_adv||0) + Number(it.honorarios_futuro_emp||0))) ?? 0) || 0)}
                    </div>
                  </div>
                </div>
              </div>
